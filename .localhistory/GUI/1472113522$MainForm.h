...
#pragma endregion
	...
	private: System::Void treeView1_DragOver(System::Object^  sender, System::Windows::Forms::DragEventArgs^  e)
	{
		...
		bool isDraggedNodeContained = containsNode(draggedNode, trvDragDrop->GetNodeAt(targetPoint));
		logger->Info(isDraggedNodeContained);

		if (isDraggedNodeContained)
		{
			trvDragDrop->Cursor = Cursors::No;
		}
		else
		{
			trvDragDrop->Cursor = Cursors::Hand;
			trvDragDrop->SelectedNode = trvDragDrop->GetNodeAt(targetPoint);
			trvDragDrop->SelectedNode->Expand();
		}
	}
